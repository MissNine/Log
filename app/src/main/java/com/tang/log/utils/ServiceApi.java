...
public class ServiceApi {
    ...
    public static void uploadLogFile(Context context, String uploadUrl, String oldFilePath) {
        try {
            URL url = new URL(Constants.upLoadUrl);
            ...
            if (con.getResponseCode() == 200) {
                Log.i("ServiceApi", "文件上传成功！上传文件为：" + oldFilePath);
            }
        } catch (Exception e) {
            e.printStackTrace();
            Log.e("ServiceApi", "文件上传失败！上传文件为：" + oldFilePath);
            Log.e("ServiceApi", "报错信息toString：" + e.toString());
        }
    }
}
